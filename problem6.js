...
const result = onlyBMWAndAudi(inventory);

function onlyBMWAndAudi(cars) {
  let arrOnlyBMWAndAudi = [],
    indexOnlyBMWAndAudi = 0;

  for (i = 0; i < cars.length; i++) {
    if (cars[i].car_make == "BMW" || cars[i].car_make == "Audi") {
      arrOnlyBMWAndAudi[indexOnlyBMWAndAudi] = cars[i];
      indexOnlyBMWAndAudi++;
    }
  }
  console.log(arrOnlyBMWAndAudi);
  console.log(arrOnlyBMWAndAudi.length);
  return arrOnlyBMWAndAudi;
}
